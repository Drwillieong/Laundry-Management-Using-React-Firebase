...
const Booking = () => {
  ...
  // Set up real-time listeners with proper data mapping
  useEffect(() => {
    ...
    const unsubscribePending = onSnapshot(pendingQuery, (snapshot) => {
      const pendingData = snapshot.docs.map(doc => {
        const data = doc.data();
        return formatBookingData(doc.id, data);
      });
  ...
  const formatBookingData = (id, data) => {
    return {
      ...
      contact: data.userContact || data.contact || "No Contact",
      email: data.userEmail || data.email || "No Email",
      address: data.userAddress || data.address || "No Address",
  ...
  const uploadPhotosToStorage = async (orderId) => {
    ...
    try {
      // Use Promise.all to upload all photos in parallel
      const uploadPromises = photoFiles.map(async (file) => {
        const storageRef = ref(storage, `orders/${orderId}/photos/${Date.now()}_${file.name}`);
  ...
  const handleCreateBooking = async (e) => {
    ...
    try {
      ...
      const bookingWithTimestamp = {
        ...
        userContact: newBooking.contact,
        userEmail: newBooking.email,
        userAddress: newBooking.address,
        totalPrice,
      ...
      const orderRef = await addDoc(collection(db, "orders"), bookingWithTimestamp);
      ...
      try {
        ...
        // Delete the order if photo upload fails
